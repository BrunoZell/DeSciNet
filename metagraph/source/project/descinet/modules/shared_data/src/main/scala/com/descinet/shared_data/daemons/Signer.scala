...
import com.descinet.shared_data.types.codecs.JsonBinaryCodec
...
trait Signer[F[_]] {
  def signAndPublish(update: DeSciNetUpdate): F[Boolean]
}
...
object Signer {
  def make[F[_] : Async : SecurityProvider : JsonSerializer](
    ...
    new Signer[F] {
      ...
      override def signAndPublish(update: DeSciNetUpdate): F[Boolean] = for {
      ...
      private def signUpdate(update: DeSciNetUpdate): F[Signed[DeSciNetUpdate]] = {
        JsonBinaryCodec[F].serialize[DeSciNetUpdate](update).flatMap { bytes =>

...
trait Publisher[F[_]] {
  ...
  def submitToTarget(update: Signed[DeSciNetUpdate]): F[Boolean]
}
...
object Publisher {
  ...
  def make[F[_] : Async](localClient: Client[F], remoteTarget: P2PContext): Publisher[F] =
    new Publisher[F] {
      ...
      override def submitToTarget(update: Signed[DeSciNetUpdate]): F[Boolean] = submit(update).run(target)

      private def submit(update: Signed[DeSciNetUpdate]): PeerResponse[F, Boolean] =
